    ...
    MaterialAdvisorContext _dbContext) : IMessageHandler<GenerateTopicMessage>
{
    public async Task HandleAsync(GenerateTopicMessage message)
    {
        try
        {
            _logger.LogDebug($"Handled {nameof(GenerateTopicMessage)}");

            var topic = await _dbContext.Topics.Include(t => t.Name).Include(t => t.Owner).AsNoTracking().SingleAsync(t => t.Id == message.TopicId);
            ValidateTopic(topic);
